...
- **Kubernetes (kubeadm) deployment ran end-to-end** (stages 7-17) on 2026-05-07..14. Worked: HA control plane, Rancher Manager, MetalLB, Nginx Ingress, kube-prometheus-stack, local-path-provisioner against Lustre. **Wiped 2026-05-14** by `stage18_wipe_for_opennebula.yml` to clear the way for OpenNebula evaluation. Stage 7-17 playbooks remain in the repo; git tag `pre-opennebula-wipe` marks the last commit before the wipe.
- **Current cluster state**: Lustre-only baseline. No workload-layer software installed beyond what Lustre needs. Ready for OpenNebula deployment (stages 19+).
...
18. `stage18_wipe_for_opennebula.yml` — **scorched-earth K8s wipe** back to Lustre baseline. Stops/disables kubelet+containerd+keepalived+haproxy, kills orphan containerd-shim/kube-proxy/flanneld/etc., dnf-removes all K8s+LB+container-runtime RPMs, deletes state dirs (`/etc/kubernetes`, `/var/lib/{etcd,kubelet,containerd}`, `/etc/cni`, `/opt/cni`, `/etc/{haproxy,keepalived}`, helm caches), drops K8s firewall ports, wipes `/mnt/lustre/k8s-volumes`. Preserves Lustre, lustre-startup.service, base cluster, and the lab CA+wildcard cert (which is copied to `/etc/pki/ujep-lab/` before wipe for OpenNebula Sunstone HTTPS reuse).
